(function () {
    ...
    app.service("$request", function ($http, $messages, $login, $guid) {
            ...
            redirect: function (url, params) {
                if (params) {
                    if (typeof params == 'object') {
                        ...
                        str += `&hash=${$guid.create()}`;
                        url += `/${btoa(str)}`;
                    } else {
                        params += `&hash=${$guid.create()}`;
                        url += `/${btoa(params)}`;
                    }
                }
                location.href = url;
    ...
    app.service("$baseService", function ($request) {
        return function (config) {
            ...
            if (typeof config.controller == "string") {
                service.redirect = function (action, params) {
                    var url = `/${config.controller}`;
        ...
        return function (options) {
            function objectMethod(type) {
                return function (success) {
                    $request.get("api/JsonUtil/ListEnums", { typeName: type }, function (enums) {
                        var o = {
                            enums,
                            names: [],
    ...
    app.factory('httpInterceptor', ['$q', '$messages','$rootScope',
        function ($q, $messages, $rootScope) {
                ...
                responseError: function (rejection) {
                    switch (rejection.status) {
                        case 500:
                            ...
                            if ($rootScope.scopeLogin) {
                                $rootScope.scopeLogin.entrando = false;
                            }
                           
                            var linhaMSG = rejection.statusText + "(" + rejection?.status + ") \n" + rejection.data.Message;
                            $messages.error(linhaMSG,20);
                            break;
                    }
